...
public enum ShipType {
    ...
    private static final Logger logShipType = LoggerFactory.getLogger(ShipType.class);
    ...
    public boolean createShip (char[][] battleField) {
        if (maxShipCount < currShipCount + 1) {
            logShipType.warn("This type of ship is full!");
        ...
        if (!isDirectionCorrect(direction)) {
            logShipType.warn("Wrong direction!");
        ...
        if (scanner.hasNextInt()) {
            ...
            logShipType.warn("Wrong coordinates!");
        ...
        for (ShipType shipType : ShipType.values()) {
            if (shipType.getShipName().equals(name)) {
                ...
                logShipType.info("Ship is complete");
                return true;
            }
        }
        return  false;
